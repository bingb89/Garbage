...
$(function() {
	...
	$(document).on('change', '#sel_c', function() {
		...
		$('#currinfo').html('');
		$('#sel_tips em').html(_touzhu[type].rule);
		$('.ui_ball').removeClass('hover');
		$('#addball').removeClass('active');
		zs_clear();
		$('.yi_num').html('').hide();
		var url = controller + '/get_config/'+type;
		$.get(url,function(result){
			$('#sel_'+type+' .sel_cc li').each(function(){
				var _this = $(this);
				var row = $(this).parents('.sel_cc').index();
				$.each(result.yilou[row],function(n,m){
					if(_this.find('a').attr('n') == n){
						_this.find('em').html(m);
					}
				});
			})
		},'json');
	...
	$('.ui_ball').click(function() {
		...
		if(type == 1){
			...
			var t = $(this).text();
			var i = $(this).parent().parent().parent().parent().attr('id');
			if ($(this).hasClass('hover') ) {
				if (index[i + t] != undefined) {
					Modal.modal({
					   'content' : '胆码和拖码相同请重新选择'
					});
					if($(this).parents('.sel_cc').index() < 0){
						$('#'+i+' .sel_cc:eq(i + t) li a').eq(i + t).removeClass('hover');
					}else{
						$(this).removeClass('hover');
					}
				} else {
	...
	$(document).on('click', '#one', function() {
		...
		if ($(this).parents('figure').attr('id') == 'index') {
			// 选号页
			$('#erdan li a').removeClass('hover');
			$('#erdan2 li a').removeClass('hover');

			var m1 = createRandom2(5, touzhu._min1, touzhu._max1);
			var m2 = createRandom2(2, touzhu._min2, touzhu._max2);
	...
	$('#addball').click(function() {
		...
		$('section#sel_1').find('.hover').each(function() {
		...
		var count1 = 0;
		var count2 = 0;
		$('section#sel_2').find('.hover').each(function() {
			if ($(this).parents('div').index() == 0) {
				if('#'+$(this).parent().parent().attr('id') == '#dant_x2' && count1 == 0){
					danzhu_arr1.push('#'+$(this).attr('n'));
					count1 ++;
				} else {
					danzhu_arr1.push($(this).attr('n'));
				...
				if('#'+$(this).parent().parent().attr('id') == '#dant_y2' && count2 == 0){
					danzhu_arr2.push('#'+$(this).attr('n'));
					count2 ++;
				} else {
					danzhu_arr2.push($(this).attr('n'));
		...
		danzhu_str1 = danzhu_str1.replace(",#","#");
		danzhu_str2 = danzhu_str2.replace(",#","#");
		...
		if(type == 1){
			if(danzhu.zs > 1){
				danzhu.title = '复式选号 '+danzhu.zs+'注 '+danzhu.je+'豆';
			}else{
				danzhu.title = '单式投注 '+danzhu.zs+'注 '+danzhu.je+'豆';
			}
		} else {
			danzhu.title = '胆拖投注 '+danzhu.zs+'注 '+danzhu.je+'豆';
		}
...
function tz_num(ball_count) {
	var type = $('#sel_c').val();
	if(type == 1){
		var c1 = C(ball_count[0], 5);
		var c2 = C(ball_count[1], 2);
		return c1*c2;
	} else {
		var b1 = ball_count[0];
		var b2 = ball_count[2];
		var c3 = C(ball_count[2], 0);
		var c4 = C(ball_count[3], 2);
		var c5 = C(ball_count[2], 1);
		var c6 = C(ball_count[3], 1);
		if(b2 == 0){
			switch(b1){
				case 1: var c1 = C(ball_count[0], 1);
					    var c2 = C(ball_count[1], 4);
					    return c1*c2*c4;
					    break;
			    case 2: var c1 = C(ball_count[0], 2);
					    var c2 = C(ball_count[1], 3);
			            return c1*c2*c4;
			            break;
			    case 3: var c1 = C(ball_count[0], 3);
						var c2 = C(ball_count[1], 2);
						return c1*c2*c4;
				case 4: var c1 = C(ball_count[0], 4);
...
function jisuan() {
	...
	if(type == 1){
		var ball_count = new Array(
			$('section#sel_1 .sel_cc').eq(0).find('.hover').length,
			$('section#sel_1 .sel_cc').eq(1).find('.hover').length
		);
	} else {
		var ball_count = new Array(
			$('section#sel_2 .sel_cc').eq(0).find('.hover').length,
			$('section#sel_2 .sel_cc').eq(1).find('.hover').length,
			$('section#sel_2 .sel_cc').eq(2).find('.hover').length,
			$('section#sel_2 .sel_cc').eq(3).find('.hover').length
		);
	}
	var zs = tz_num(ball_count);
	var je = zs * dj;
	if(zs != undefined && je != NaN){
...
function rand_ball(num) {
	...
	for ( var i = 0; i < num; i++) {
		redball[i] = createRandom2(5, touzhu._min1, touzhu._max1);
		blueball[i] = createRandom2(2, touzhu._min2, touzhu._max2);
	}
