...
$(function(){
  ...
  $('#numberCheck').keyup(function(){
    ...
    if(reg.test(number)){
      ...
      var errorMsg = " * 请输入8位数字投注机号 ";
      $('#numberCheck').next().html(errorMsg);
      return false;
    }
  });

  $('#numberSerial').keyup(function(){
    //如果表单为空，提示用户
    var number = $(this).val();
    var reg = /^\d+$/;
    if(reg.test(number)){
